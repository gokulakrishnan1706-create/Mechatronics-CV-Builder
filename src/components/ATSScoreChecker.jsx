...
export default function ATSScoreChecker() {
  ...
  const handleFixCV = () => {
    // Pass missing keywords context to the Part-Time CV generator
    if (results?.missingKeywords) {
      const context = results.missingKeywords
        .filter(k => k.importance === 'high')
        .map(k => k.keyword)
        .join(', ');
      // Navigate to builder with context
      window.location.hash = `#builder?ats_keywords=${encodeURIComponent(context)}&sector=${results.detectedSector}`;
    }
  };
